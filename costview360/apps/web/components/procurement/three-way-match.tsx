...
export function ThreeWayMatchView() {
  ...
  return (
    <div className="bg-white border-2 border-navy-800 shadow-brutal overflow-hidden shadow-sm space-y-4">
      ...
      <div className="p-3 bg-navy-800 border-b-[3px] border-navy-800 flex items-center justify-between overflow-x-auto gap-2">
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className={`px-3.5 py-2 border-2 border-navy-800 text-xs font-black uppercase tracking-wider transition-all flex items-center gap-1.5 ${
              subTab === "requisitions"
          ...
          >
            ...
            {requisitions.filter((r) => r.status === "Pending Approval").length > 0 && (
              <span className="text-[11px] bg-amber-400 text-navy-800 px-1.5 py-0.5 font-mono font-black border border-navy-800">
      ...
      {subTab === "match" && (
        <div>
          <div className="p-4 border-b-2 border-navy-800 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-black text-navy-800 flex items-center gap-2">
                <span>Three-Way Financial Match Engine</span>
                <span className="text-xs bg-emerald-100 text-emerald-900 px-2.5 py-1 font-mono font-black border-2 border-navy-800">
                  PO ⇄ GRN ⇄ Invoice
                </span>
              </h3>
              <p className="text-xs font-bold text-navy-800/70 mt-0.5">
                Automated quantity and unit rate cross-validation per PRD Section 2.
              </p>
            </div>
            <button
              onClick={() => setIsNewInvoiceOpen(true)}
              className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px] flex items-center gap-1.5"
            >
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {records.map((rec) => (
              <div key={rec.id} className="p-4 hover:bg-cream-100/40 transition-colors">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-black text-navy-800 text-xs">{rec.supplierName}</span>
                      <span className="text-navy-800/40">·</span>
                      <span className="text-xs font-bold text-navy-800/70">{rec.itemDescription}</span>
                      <span className="text-navy-800/40">·</span>
                      <span className="text-xs text-navy-800 bg-cream-100 px-2.5 py-0.5 font-mono font-bold border-2 border-navy-800">
                        Terms: {rec.paymentTerms}
                      </span>
                    </div>

                    {/* 3 Steps Matrix */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3 bg-cream-100/60 p-3 border-2 border-navy-800">
                      {/* Step 1: PO */}
                      <div>
                        <div className="text-xs font-black text-navy-800/60 uppercase tracking-wider mb-1">
                          1. Purchase Order
                        </div>
                        <div className="text-xs font-mono text-navy-800 font-bold">
                          Qty: <span className="font-black text-navy-800">{rec.poQty}</span> @ {formatCurrency(rec.poRate, currency)}
                        </div>
                        <div className="text-xs font-mono font-black text-navy-800 mt-0.5">
                      ...
                      {/* Step 2: GRN */}
                      <div>
                        <div className="text-xs font-black text-navy-800/60 uppercase tracking-wider mb-1 flex items-center justify-between">
                        ...
                        <div className="text-xs text-navy-800/80 font-bold mt-0.5">
                      ...
                      <div>
                        ...
                        <div className="text-xs font-mono text-navy-800 font-bold">
                          Billed: <span className="font-black text-navy-800">{rec.invoiceQtyBilled}</span> @ {formatCurrency(rec.invoiceRateBilled, currency)}
                        </div>
                        <div className="text-xs font-mono font-black text-navy-800 mt-0.5">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {requisitions.map((req) => (
              <div key={req.id} className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-cream-100/40 px-3 border-b-2 border-navy-800/10 transition-colors">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono font-black text-navy-800 text-xs">{req.reqNumber}</span>
                    <span className="text-navy-800/40">·</span>
                    <span className="font-black text-navy-800 text-xs">{req.itemDescription}</span>
                    <span className="text-navy-800/40">·</span>
                    <span className={`px-2 py-0.5 text-xs font-black uppercase tracking-wider border-2 ${
                      req.urgency === "Critical"
                        ? "bg-red-100 text-red-900 border-navy-800"
                        : req.urgency === "High"
                        ? "bg-amber-100 text-amber-900 border-navy-800"
                        : "bg-white text-navy-800 border-navy-800"
                    }`}>
                      {req.urgency} Urgency
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-navy-800/70 font-medium">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2.5 py-1 text-xs font-black uppercase tracking-wider border-2 ${
                    req.status === "Approved"
                      ? "bg-emerald-100 text-emerald-900 border-navy-800"
                      : req.status === "Rejected"
                      ? "bg-red-100 text-red-900 border-navy-800"
                      : "bg-amber-100 text-amber-900 border-navy-800"
                  }`}>
                  ...
                  {req.status === "Pending Approval" && (
                    <div className="flex items-center gap-2 ml-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[1px] active:translate-y-[1px] flex items-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[1px] active:translate-y-[1px] flex items-center gap-1"
                      >
                        <X className="w-3 h-3" />
                        <span>Reject</span>
                      </button>
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between border-b-2 border-navy-800 pb-3">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-blue-600 hover:bg-blue-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px] flex items-center gap-1.5"
            >
              <Plus className="w-3.5 h-3.5" />
          ...
          <div className="space-y-4">
            {enquiries.map((enq) => (
              <div key={enq.id} className="bg-cream-100 p-4 border-2 border-navy-800 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-mono font-bold text-blue-700 text-xs">{enq.enquiryNumber}</span>
                    <h4 className="text-xs font-black text-navy-800 mt-0.5">
                      {enq.itemDescription} ({enq.quantity} {enq.unit})
                    </h4>
                  </div>
                  <span className="text-xs bg-white text-navy-800 px-2.5 py-1 font-mono font-black border-2 border-navy-800">
                    {enq.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {enq.suppliersQuoted.map((quote, idx) => (
                    <div key={idx} className="p-3 bg-white border-2 border-navy-800 flex flex-col justify-between shadow-brutal-sm">
                      <div>
                        <div className="text-xs font-black text-navy-800">{quote.name}</div>
                        <div className="text-xs font-mono text-emerald-700 font-bold mt-1">
                          {formatCurrency(quote.quoteRate, currency)} / {enq.unit}
                        </div>
                        <div className="text-xs text-navy-800/70 font-medium mt-0.5">
                          Delivery: {quote.leadDays} Days Lead
                        </div>
                      </div>
                      <button
                        onClick={() => alert(`Supplier ${quote.name} selected. Converting RFQ to Purchase Order.`)}
                        className="mt-3 w-full py-2 bg-mustard-400 hover:bg-mustard-500 text-navy-800 border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[1px] active:translate-y-[1px] flex items-center justify-center gap-1"
                      >
                        <span>Award & Create PO</span>
                        <ArrowRight className="w-3 h-3 text-navy-800" />
                      </button>
                    </div>
                  ))}
                </div>
      ...
      {subTab === "invoices" && (
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between border-b-2 border-navy-800 pb-3">
            ...
            <button
              onClick={() => setIsNewInvoiceOpen(true)}
              className="px-3.5 py-2 bg-purple-600 hover:bg-purple-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px] flex items-center gap-1.5"
            >
              <Plus className="w-3.5 h-3.5" />
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {records.map((r) => (
              <div key={r.id} className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-cream-100/40 px-3 border-b-2 border-navy-800/10 transition-colors">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono font-bold text-purple-700 text-xs">{r.invoiceNumber}</span>
                    <span className="text-navy-800/40">·</span>
                    <span className="font-black text-navy-800 text-xs">{r.supplierName}</span>
                    <span className="text-navy-800/40">·</span>
                    <span className="font-mono text-emerald-700 text-xs font-black">
                      {formatCurrency(r.invoiceTotal, currency)}
                    </span>
                  </div>
                  <div className="text-xs text-navy-800/70 font-medium">
                ...
                <div className="flex items-center gap-2">
                  {r.discrepancyReason ? (
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-amber-400 hover:bg-amber-500 text-navy-800 border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[1px] active:translate-y-[1px]"
                    >
                      Request Credit Note
                    </button>
                  ) : (
                    <span className="px-2.5 py-1 bg-emerald-100 text-emerald-900 border-2 border-navy-800 text-xs font-black uppercase tracking-wider">
                      Verified & Cleared
                    </span>
                  )}
                </div>
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {records.map((r) => (
              <div key={r.id} className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-cream-100/40 px-3 border-b-2 border-navy-800/10 transition-colors">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono font-bold text-navy-800 text-xs">{r.supplierName}</span>
                    <span className="text-navy-800/40">·</span>
                    <span className="font-mono text-emerald-700 text-xs font-black">
                      {formatCurrency(r.invoiceTotal, currency)}
                    </span>
                  ...
                  {r.matchStatus !== "Paid" && (
                    <>
                      ...
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[1px] active:translate-y-[1px] flex items-center gap-1"
                      >
                        <CreditCard className="w-3.5 h-3.5" />
                        <span>Process</span>
                      </button>
      ...
      {isNewReqOpen && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white border-2 border-navy-800 max-w-md w-full p-6 shadow-brutal">
            <div className="border-b-2 border-navy-800 pb-3">
              <h3 className="text-sm font-black text-navy-800 uppercase tracking-wide">Raise New Material Requisition</h3>
            </div>
            <form onSubmit={handleCreateRequisition} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Item Description</label>
                <input
                  type="text"
                  required
                  value={reqDesc}
                  ...
                  className="w-full bg-white border-2 border-navy-800 px-3 py-2 text-xs font-bold text-navy-800 focus:outline-none focus:shadow-brutal-sm"
                />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Quantity</label>
                  <input
                    type="number"
                    required
                    value={reqQty}
                    onChange={(e) => setReqQty(Number(e.target.value))}
                    className="w-full bg-white border-2 border-navy-800 px-3 py-2 text-xs text-navy-800 font-mono font-bold focus:outline-none focus:shadow-brutal-sm"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Unit</label>
                  <input
                    type="text"
                    required
                    value={reqUnit}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Estimated Cost (₦)</label>
                  <input
                    type="number"
                    required
                    value={reqEstCost}
                ...
                <div>
                  <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Urgency</label>
                  <select
                    value={reqUrgency}
                    onChange={(e) => setReqUrgency(e.target.value as Requisition["urgency"])}
                    className="w-full bg-white border-2 border-navy-800 px-3 py-2 text-xs font-black text-navy-800 uppercase focus:outline-none focus:shadow-brutal-sm"
                  >
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t-2 border-navy-800/10">
                <button
                  type="button"
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px]"
                >
                  Submit Requisition
                </button>
              </div>
      ...
      {isNewEnquiryOpen && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white border-2 border-navy-800 max-w-md w-full p-6 shadow-brutal">
            <div className="border-b-2 border-navy-800 pb-3">
              <h3 className="text-sm font-black text-navy-800 uppercase tracking-wide">Create Supplier Price Enquiry (RFQ)</h3>
            </div>
            <form onSubmit={handleCreateEnquiry} className="mt-4 space-y-3">
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t-2 border-navy-800/10">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px]"
                >
                  Send for Pricing
                </button>
              </div>
      ...
      {isNewInvoiceOpen && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white border-2 border-navy-800 max-w-md w-full p-6 shadow-brutal">
            <div className="border-b-2 border-navy-800 pb-3">
              <h3 className="text-sm font-black text-navy-800 uppercase tracking-wide">Log Supplier Invoice</h3>
            </div>
            <form onSubmit={handleCreateInvoice} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Invoice Number</label>
                <input
                  type="text"
                  required
                  value={invNumber}
              ...
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Supplier Name</label>
                <input
                  type="text"
                  required
                  value={invSupplier}
              ...
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Total Invoiced Amount (₦)</label>
                <input
                  type="number"
                  required
                  value={invAmount}
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t-2 border-navy-800/10">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px]"
                >
      ...
      {isProcessPaymentOpen && selectedRecord && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white border-2 border-navy-800 max-w-md w-full p-6 shadow-brutal">
            <div className="border-b-2 border-navy-800 pb-3">
              <h3 className="text-sm font-black text-navy-800 uppercase tracking-wide">Process Supplier Disbursement</h3>
              <p className="text-xs font-bold text-navy-800/70 mt-1">
            ...
            <form onSubmit={handleConfirmPayment} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Payment Method / Bank Account</label>
                <select
                  value={paymentMethod}
              ...
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Payment Reference / Transaction ID</label>
                <input
                  type="text"
                  required
      ...
      {isRateDeliveryOpen && selectedRecord && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white border-2 border-navy-800 max-w-md w-full p-6 shadow-brutal">
            <div className="border-b-2 border-navy-800 pb-3">
              <h3 className="text-sm font-black text-navy-800 uppercase tracking-wide">Rate This Delivery Quality</h3>
              <p className="text-xs font-bold text-navy-800/70 mt-1">
            ...
            <form onSubmit={handleSaveDeliveryRating} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Quality Assessment (1 - 5 Stars)</label>
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      ...
                      className={`p-2 border-2 border-navy-800 flex items-center gap-1.5 shadow-brutal-sm transition-all ${
                        ratingScore >= star
                          ? "bg-amber-400 text-navy-800 font-black"
                          : "bg-white text-navy-800/40"
                      }`}
                    >
                      <Star className={`w-4 h-4 ${ratingScore >= star ? "fill-navy-800 text-navy-800" : ""}`} />
              ...
              <div>
                <label className="block text-xs font-bold text-navy-800 mb-1 uppercase tracking-wider">Inspector Note / Site Observation</label>
                <textarea
                  rows={3}
                  value={ratingNote}
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t-2 border-navy-800/10">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-amber-400 hover:bg-amber-500 text-navy-800 border-2 border-navy-800 text-xs font-black uppercase tracking-wider shadow-brutal-sm transition-all active:translate-x-[2px] active:translate-y-[2px]"
                >
                  Save Quality Rating
                </button>
              </div>
