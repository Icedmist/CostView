...
export function ThreeWayMatchView({
  ...
  // Load live procurement matches from Supabase
  useEffect(() => {
    let isMounted = true;
    (async () => {
      try {
        ...
        const { data } = await supabase
          ...
          .select(`
            id, is_quantity_matched, is_rate_matched, variance_amount, flagged_reason, created_at,
            supplier_invoices:invoice_id (
              id, invoice_number, billed_amount, match_status, invoice_date, notes,
              purchase_orders:purchase_order_id (
                id, po_number, total_amount, status, notes,
                suppliers:supplier_id ( name ),
                po_items ( description, quantity, agreed_rate, line_total )
              )
            ),
            goods_received_notes:grn_id (
              id, grn_number, remarks,
              grn_items ( quantity_ordered, quantity_delivered, discrepancy_qty )
            )
          `);

        if (isMounted && data && data.length > 0) {
          const mapped: ThreeWayMatchRecord[] = data.map((d: any) => {
            const inv = d.supplier_invoices;
            const po = inv?.purchase_orders;
            const sup = po?.suppliers;
            const poItem = po?.po_items?.[0];
            const grn = d.goods_received_notes;
            const grnItem = grn?.grn_items?.[0];

            const poQty = Number(poItem?.quantity || 100);
            const poRate = Number(poItem?.agreed_rate || 0);
            const poTotal = Number(po?.total_amount || (poQty * poRate));
            const grnQty = Number(grnItem?.quantity_delivered || poQty);
            const invTotal = Number(inv?.billed_amount || (poQty * poRate));

            let matchStatus: ThreeWayMatchRecord["matchStatus"] = "Matched";
            if (inv?.match_status === "Paid") matchStatus = "Paid";
            else if (d.variance_amount > 0 || !d.is_quantity_matched || inv?.match_status === "Discrepancy") matchStatus = "Discrepancy";

            return {
              id: d.id,
              poNumber: po?.po_number || "PO-2026-088",
              supplierName: sup?.name || "Supplier",
              itemDescription: poItem?.description || "Construction Materials",
              poQty,
              poRate,
              poTotal,
              paymentTerms: "Pay After Delivery",
              grnNumber: grn?.grn_number || "GRN-0941",
              grnQtyReceived: grnQty,
              invoiceNumber: inv?.invoice_number || "INV-001",
              invoiceQtyBilled: matchStatus === "Discrepancy" ? poQty : grnQty,
              invoiceRateBilled: poRate,
              invoiceTotal: invTotal,
              matchStatus,
              discrepancyReason: d.flagged_reason || (matchStatus === "Discrepancy" ? "Quantity variance detected between invoice and site delivery" : undefined),
              paymentLocked: matchStatus === "Discrepancy",
              deliveryQualityScore: 5.0,
            };
          });
          setRecords(mapped);
        }
      } catch (err) {
        console.warn("Failed to load live three_way_matches from Supabase", err);
      }
    })();
    return () => {
      isMounted = false;
  ...
  const handleResolveDiscrepancy = async (id: string) => {
    ...
    try {
      ...
      await supabase
        .from("three_way_matches")
        .update({ is_quantity_matched: true, variance_amount: 0, flagged_reason: null })
        .eq("id", id);
    } catch (e) {
      console.warn("Failed to update three_way_matches", e);
    }
  };
    ...
    <div className="bg-white/90 backdrop-blur-md border border-slate-200/80 rounded-xl shadow-xs rounded-xl overflow-hidden space-y-4">
          ...
          <div className="p-4 border-b border-slate-200/80 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                <span className="text-xs bg-emerald-100 text-emerald-900 px-2.5 py-1 font-mono font-bold border border-slate-200/80">
                  PO ⇄ GRN ⇄ Invoice
                </span>
              </h3>
              <p className="text-xs font-bold text-slate-900/70 mt-0.5">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-bold text-slate-900 text-xs">{rec.supplierName}</span>
                      <span className="text-slate-900/40">·</span>
                      <span className="text-xs font-bold text-slate-900/70">{rec.itemDescription}</span>
                      <span className="text-slate-900/40">·</span>
                      <span className="text-xs text-slate-900 bg-white px-2.5 py-0.5 font-mono font-bold border border-slate-200/80">
                        Terms: {rec.paymentTerms}
                      </span>
                    </div>

                    {/* 3 Steps Matrix */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3 bg-white/60 p-3 border border-slate-200/80">
                      {/* Step 1: PO */}
                      <div>
                        <div className="text-xs font-bold text-slate-900/60 uppercase tracking-wider mb-1">
                          1. Purchase Order
                        </div>
                        <div className="text-xs font-mono text-slate-900 font-bold">
                          Qty: <span className="font-bold text-slate-900">{rec.poQty}</span> @ {formatCurrency(rec.poRate, currency)}
                        </div>
                      ...
                      {/* Step 2: GRN */}
                      <div>
                        <div className="text-xs font-bold text-slate-900/60 uppercase tracking-wider mb-1 flex items-center justify-between">
                        ...
                        <div className="text-xs font-mono text-slate-900 font-bold">
                          Received: <span className={`font-bold ${rec.grnQtyReceived !== rec.poQty ? "text-amber-600" : "text-emerald-700"}`}>
                            {rec.grnQtyReceived}
                          </span>
                        </div>
                        <div className="text-xs text-slate-900/80 font-bold mt-0.5">
                      ...
                      <div>
                        ...
                        <div className="text-xs font-mono text-slate-900 font-bold">
                          Billed: <span className="font-bold text-slate-900">{rec.invoiceQtyBilled}</span> @ {formatCurrency(rec.invoiceRateBilled, currency)}
                        </div>
                        <div className="text-xs font-mono font-bold text-slate-900 mt-0.5">
                    ...
                    {rec.discrepancyReason && (
                      <div className="mt-2 flex items-center gap-2 p-2.5 bg-red-100 border border-rose-300 rounded-xl text-red-900 text-xs font-bold">
                  ...
                  <div className="flex flex-wrap items-center gap-2 lg:self-center">
                    {rec.paymentLocked ? (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-100 border border-rose-300 rounded-xl text-red-900 text-xs font-bold tracking-wide">
                        <Lock className="w-3.5 h-3.5 text-red-700" />
                        <span>Payment Locked</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200/80 bg-emerald-950/80 border border-emerald-800 text-emerald-300 text-xs font-semibold">
                    ...
                    {rec.paymentLocked && (
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 bg-white hover:bg-white text-slate-900 border border-slate-200/80 text-xs font-semibold transition-colors"
                      >
                    ...
                    {!rec.paymentLocked && rec.matchStatus !== "Paid" && (
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border border-slate-200/80 text-xs font-semibold shadow-sm transition-colors"
                      >
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-900/70 mt-0.5">
                Site Engineer manual orders or low-stock automated triggers (PRD Section 2.2).
              </p>
            </div>
            <button
              onClick={() => setIsNewReqOpen(true)}
              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border border-slate-200/80 text-xs font-semibold shadow-sm transition-colors flex items-center gap-1.5"
            >
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {requisitions.map((req) => (
              <div key={req.id} className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-white/40 px-3 border-b border-slate-200/80/10 transition-colors">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono font-bold text-slate-900 text-xs">{req.reqNumber}</span>
                    <span className="text-slate-900/40">·</span>
                    <span className="font-bold text-slate-900 text-xs">{req.itemDescription}</span>
                    <span className="text-slate-900/40">·</span>
                    <span className={`px-2 py-0.5 text-xs font-bold tracking-wide border ${
                      req.urgency === "Critical"
                        ? "bg-red-100 text-red-900 border-slate-200/80"
                        : req.urgency === "High"
                        ? "bg-amber-100 text-amber-900 border-slate-200/80"
                        : "bg-white text-slate-900 border-slate-200/80"
                    }`}>
                      {req.urgency} Urgency
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-slate-900/70 font-medium">
                    <span>Qty: <strong className="text-slate-900">{req.quantity} {req.unit}</strong></span>
                    <span>·</span>
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2.5 py-1 text-xs font-bold tracking-wide border ${
                    req.status === "Approved"
                      ? "bg-emerald-100 text-emerald-900 border-slate-200/80"
                      : req.status === "Rejected"
                      ? "bg-red-100 text-red-900 border-slate-200/80"
                      : "bg-amber-100 text-amber-900 border-slate-200/80"
                  }`}>
                  ...
                  {req.status === "Pending Approval" && (
                    <div className="flex items-center gap-2 ml-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98] flex items-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98] flex items-center gap-1"
                      >
                        <X className="w-3 h-3" />
                        <span>Reject</span>
                      </button>
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-blue-600 hover:bg-blue-500 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98] flex items-center gap-1.5"
            >
              <Plus className="w-3.5 h-3.5" />
          ...
          <div className="space-y-4">
            {enquiries.map((enq) => (
              <div key={enq.id} className="bg-white p-4 border border-slate-200/80 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs bg-white text-slate-900 px-2.5 py-1 font-mono font-bold border border-slate-200/80">
                    {enq.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {enq.suppliersQuoted.map((quote, idx) => (
                    <div key={idx} className="p-3 bg-white border border-slate-200/80 flex flex-col justify-between shadow-xs rounded-xl">
                      <div>
                        <div className="text-xs font-bold text-slate-900">{quote.name}</div>
                        <div className="text-xs font-mono text-emerald-700 font-bold mt-1">
                          {formatCurrency(quote.quoteRate, currency)} / {enq.unit}
                        </div>
                        <div className="text-xs text-slate-900/70 font-medium mt-0.5">
      ...
      {subTab === "invoices" && (
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
            ...
            <button
              onClick={() => setIsNewInvoiceOpen(true)}
              className="px-3.5 py-2 bg-purple-600 hover:bg-purple-500 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98] flex items-center gap-1.5"
            >
              <Plus className="w-3.5 h-3.5" />
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {records.map((r) => (
              <div key={r.id} className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-white/40 px-3 border-b border-slate-200/80/10 transition-colors">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="text-xs text-slate-900/70 font-medium">
                    Linked PO: <strong className="text-slate-900">{r.poNumber}</strong> · GRN: <strong className="text-slate-900">{r.grnNumber}</strong>
                  </div>
                  {r.discrepancyReason && (
                ...
                <div className="flex items-center gap-2">
                  {r.discrepancyReason ? (
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-amber-400 hover:bg-amber-500 text-slate-900 border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98]"
                    >
                      Request Credit Note
                    </button>
                  ) : (
                    <span className="px-2.5 py-1 bg-emerald-100 text-emerald-900 border border-slate-200/80 text-xs font-bold tracking-wide">
                      Verified & Cleared
                    </span>
                  )}
                </div>
          ...
          <div className="divide-y-2 divide-navy-800/20">
            {records.map((r) => (
              <div key={r.id} className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-white/40 px-3 border-b border-slate-200/80/10 transition-colors">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono font-bold text-slate-900 text-xs">{r.supplierName}</span>
                    <span className="text-slate-900/40">·</span>
                    <span className="font-mono text-emerald-700 text-xs font-bold">
                      {formatCurrency(r.invoiceTotal, currency)}
                    </span>
                  ...
                  {r.matchStatus !== "Paid" && (
                    <>
                      ...
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98] flex items-center gap-1"
                      >
                        <CreditCard className="w-3.5 h-3.5" />
                        <span>Process</span>
                      </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-xl max-w-md w-full p-6 shadow-xl">
            <div className="border-b border-slate-200/80 pb-3">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide">Raise New Material Requisition</h3>
            </div>
            <form onSubmit={handleCreateRequisition} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Item Description</label>
                <input
                  type="text"
                  required
                  value={reqDesc}
                  ...
                  className="w-full bg-white border border-slate-200/80 px-3 py-2 text-xs font-bold text-slate-900 focus:outline-none focus:shadow-xs rounded-xl"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-200/80 px-3 py-2 text-xs text-slate-900 font-mono font-bold focus:outline-none focus:shadow-xs rounded-xl"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Estimated Cost (₦)</label>
                  <input
                    type="number"
                    required
                    value={reqEstCost}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Urgency</label>
                  <select
                    value={reqUrgency}
                    onChange={(e) => setReqUrgency(e.target.value as Requisition["urgency"])}
                    className="w-full bg-white border border-slate-200/80 px-3 py-2 text-xs font-bold text-slate-900 uppercase focus:outline-none focus:shadow-xs rounded-xl"
                  >
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-200/80/10">
                <button
                  type="button"
                  onClick={() => setIsNewReqOpen(false)}
                  className="px-4 py-2 bg-white hover:bg-[#f5f5f5] text-slate-900 border border-slate-200/80 rounded-md text-xs font-medium uppercase tracking-wider shadow-xs rounded-xl"
                >
                  Cancel
                </button>
                <button
      ...
      {isNewEnquiryOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-xl max-w-md w-full p-6 shadow-xl">
            <div className="border-b border-slate-200/80 pb-3">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide">Create Supplier Price Enquiry (RFQ)</h3>
            </div>
            <form onSubmit={handleCreateEnquiry} className="mt-4 space-y-3">
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-200/80/10">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98]"
                >
                  Send for Pricing
                </button>
              </div>
      ...
      {isNewInvoiceOpen && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-xl max-w-md w-full p-6 shadow-xl">
            <div className="border-b border-slate-200/80 pb-3">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide">Log Supplier Invoice</h3>
            </div>
            <form onSubmit={handleCreateInvoice} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Invoice Number</label>
                <input
                  type="text"
                  required
                  value={invNumber}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Supplier Name</label>
                <input
                  type="text"
                  required
                  value={invSupplier}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Total Invoiced Amount (₦)</label>
                <input
                  type="number"
                  required
                  value={invAmount}
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-200/80/10">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98]"
                >
      ...
      {isProcessPaymentOpen && selectedRecord && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-xl max-w-md w-full p-6 shadow-xl">
            <div className="border-b border-slate-200/80 pb-3">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide">Process Supplier Disbursement</h3>
              <p className="text-xs font-bold text-slate-900/70 mt-1">
                Disbursing <strong className="text-emerald-700 font-mono font-bold">{formatCurrency(selectedRecord.invoiceTotal, currency)}</strong> to {selectedRecord.supplierName}.
              </p>
            </div>

            <form onSubmit={handleConfirmPayment} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Payment Method / Bank Account</label>
                <select
                  value={paymentMethod}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Payment Reference / Transaction ID</label>
                <input
                  type="text"
                  required
      ...
      {isRateDeliveryOpen && selectedRecord && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 rounded-xl max-w-md w-full p-6 shadow-xl">
            <div className="border-b border-slate-200/80 pb-3">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wide">Rate This Delivery Quality</h3>
              <p className="text-xs font-bold text-slate-900/70 mt-1">
            ...
            <form onSubmit={handleSaveDeliveryRating} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Quality Assessment (1 - 5 Stars)</label>
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      ...
                      className={`p-2 border border-slate-200/80 flex items-center gap-1.5 shadow-xs rounded-xl transition-all ${
                        ratingScore >= star
                          ? "bg-amber-400 text-slate-900 font-bold"
                          : "bg-white text-slate-900/40"
                      }`}
                    >
                      <Star className={`w-4 h-4 ${ratingScore >= star ? "fill-navy-800 text-slate-900" : ""}`} />
              ...
              <div>
                <label className="block text-xs font-bold text-slate-900 mb-1 uppercase tracking-wider">Inspector Note / Site Observation</label>
                <textarea
                  rows={3}
                  value={ratingNote}
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-200/80/10">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-amber-400 hover:bg-amber-500 text-slate-900 border border-slate-200/80 text-xs font-bold tracking-wide shadow-xs rounded-xl transition-all active:scale-[0.98]"
                >
                  Save Quality Rating
                </button>
              </div>
