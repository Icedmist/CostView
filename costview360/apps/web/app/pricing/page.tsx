    ...
    <div className="min-h-screen bg-[#FFFDF0] text-navy-800 font-sans">
      ...
      <header className="border-b-[3px] border-navy-800 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-14 text-center">
          <div className="inline-block bg-[#FFD23F] border-[3px] border-navy-800 px-4 py-2 font-black uppercase text-sm shadow-brutal-sm">Naira Friendly</div>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {TIERS.map((t) => (
            <div key={t.name} className={`${t.dark ? "bg-navy-800 text-white" : "bg-white"} border-[3px] border-navy-800 shadow-brutal p-8 relative flex flex-col`}>
              {t.badge && <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#FFD23F] text-navy-800 border-[3px] border-navy-800 px-4 py-1.5 font-black uppercase text-xs whitespace-nowrap">{t.badge}</div>}
              ...
              <ul className="mt-6 space-y-3 flex-1">
                {t.feats.map((f) => (
                  <li key={f} className="flex items-center gap-3 text-sm font-bold">
                    <span className={`w-6 h-6 border-2 border-navy-800 flex items-center justify-center shrink-0 ${t.dark ? "bg-[#FFD23F] text-navy-800" : "bg-navy-800 text-white"}`}><CheckCircle2 className="w-4 h-4" /></span>{f}
                  </li>
                ))}
              </ul>
              <Link href={t.href} className={`mt-8 text-center py-4 border-[3px] border-navy-800 font-black uppercase text-sm shadow-brutal-sm ${t.dark ? "bg-[#FFD23F] text-navy-800" : "bg-white text-navy-800"}`}>{t.cta}</Link>
            </div>
          ))}
        </div>

        <div className="mt-12 max-w-5xl mx-auto bg-white border-[3px] border-navy-800 shadow-brutal overflow-hidden">
          <div className="p-6 border-b-[3px] border-navy-800 bg-[#FFFDF0]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y-2 divide-navy-800/10">
                {COMPARISON.map((row) => (
                  <tr key={row[0]} className="hover:bg-[#FFFDF0]">
      ...
      <section className="bg-white border-y-[3px] border-navy-800">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 grid md:grid-cols-3 gap-6">
          <div className="bg-[#FFFDF0] border-[3px] border-navy-800 p-6 shadow-brutal"><Zap className="w-8 h-8 mb-3" /><h4 className="font-black uppercase">Monthly, No Lock-in</h4><p className="text-sm font-bold text-navy-800/60 mt-2">Cancel anytime. Invoices in ₦, card or bank transfer.</p></div>
          <div className="bg-[#FFD23F] border-[3px] border-navy-800 p-6 shadow-brutal"><ShieldCheck className="w-8 h-8 mb-3" /><h4 className="font-black uppercase">RLS & Audit</h4><p className="text-sm font-bold mt-2">All tiers get Row-Level Security. Growth+ gets full audit export.</p></div>
          <div className="bg-navy-800 text-white border-[3px] border-navy-800 p-6 shadow-brutal"><Building2 className="w-8 h-8 mb-3 text-mustard-400" /><h4 className="font-black uppercase">Priority Support</h4><p className="text-sm font-bold text-white/70 mt-2">Priority on Growth, dedicated on Enterprise — same timezone — remote & on-site.</p></div>
        </div>
      </section>
