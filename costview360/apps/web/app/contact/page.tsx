    ...
    <div className="min-h-screen bg-cream-100 text-navy-800 font-sans">
      ...
      <header className="border-b-[3px] border-navy-800 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 md:py-14">
          ...
          <h1 className="mt-4 text-4xl md:text-5xl font-black tracking-tighter leading-none">Let’s Talk<br /><span className="bg-mustard-400 border-[3px] border-navy-800 px-2">Your Next Build.</span></h1>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Engr. Babatunde" className="w-full bg-cream-100 border-2 border-navy-800 px-3 py-3 text-sm font-bold" />
                </div>
                <div>
                  <label className="block text-xs font-black uppercase tracking-widest mb-1">Work Email</label>
                  <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@firm.ng" className="w-full bg-cream-100 border-2 border-navy-800 px-3 py-3 text-sm font-bold" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-black uppercase tracking-widest mb-1">Company</label>
                <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Julius Berger / Eko Atlantic" className="w-full bg-cream-100 border-2 border-navy-800 px-3 py-3 text-sm font-bold" />
              </div>
              <div>
                <label className="block text-xs font-black uppercase tracking-widest mb-1">Message</label>
                <textarea required rows={5} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="We run 2 sites, 40 workers, BOQ in Excel, need 3-way match..." className="w-full bg-cream-100 border-2 border-navy-800 px-3 py-3 text-sm font-bold" />
              </div>
          ...
          <div className="space-y-4">
            <div className="bg-navy-800 text-white border-[3px] border-navy-800 p-6 shadow-brutal">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className="bg-mustard-400 text-navy-800 px-3 py-1 text-xs font-black border-2 border-navy-800">HQ: VI</span>
            ...
            <div className="bg-cream-100 border-[3px] border-navy-800 p-6 shadow-brutal">
              ...
              <Link href="/register" className="mt-4 inline-block px-5 py-3 bg-mustard-400 border-2 border-navy-800 font-black uppercase text-sm shadow-brutal-sm">Create Workspace →</Link>
            </div>
          </div>
        </div>
      </section>
